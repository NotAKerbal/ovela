...
export function SiteHeader({ onManageNavigate }: { onManageNavigate?: () => void }) {
  ...
  return <header className="site-header"><Link className="brand" href="/" aria-label="Ovela home"><span className="brand-mark" aria-hidden="true"><i /><i /><i /><i /></span><span>Ovela</span></Link>
    <div className="header-actions">{person && !person.suspended && <><Link className={pathname === '/' ? 'header-active' : ''} href="/">Home</Link>{person.role === 'admin' && <Link className={pathname.startsWith('/manage') ? 'header-active' : ''} href="/manage" onNavigate={event => { if (onManageNavigate) { event.preventDefault(); onManageNavigate(); } }}>Manage</Link>}<Link className="account-link" href="/account">{person.name}</Link></>}</div></header>;
}
