...
export default defineSchema({
  people: defineTable({ authId: v.string(), name: v.string(), email: v.string(), role, suspended: v.boolean(), appIds: v.array(v.id('applications')) }).index('by_auth', ['authId']).index('by_email', ['email']),
