      ...
      user: {
        onCreate: async (ctx, user) => {
          ...
          const invitation = existing
            ? await ctx.db
                .query("invitations")
                .withIndex("by_token", (q) => q.eq("tokenHash", enrollmentHash))
                .unique()
            : null;
          ...
          if (
            !existing &&
            (!process.env.OVELA_SETUP_TOKEN ||
              enrollmentHash !== hashToken(process.env.OVELA_SETUP_TOKEN))
          ) {
          ...
          if (!existing) {
            const defaults = [
              ...
              {
                name: "Media",
                description: "Something good to watch.",
                icon: "media" as const,
                color: "#aebfc6",
                ink: "#456575",
              },
              {
                name: "Notes",
                description: "Room for your ideas.",
                icon: "notes" as const,
                color: "#d4b7a7",
                ink: "#875e4a",
              },
          ...
          await ctx.db.insert("people", {
            ...
            name: user.name,
            email,
            role: existing ? invitation!.role : "admin",
            suspended: false,
            appIds: invitation?.appIds ?? [],
          });
          if (invitation)
            await ctx.db.patch(invitation._id, { consumed: true });
        },
      },
      session: {
        onCreate: async (ctx, session) => {
          const person = await ctx.db
            .query("people")
            .withIndex("by_auth", (q) => q.eq("authId", session.userId))
            .unique();
  ...
  ({
    ...
    hooks: {
      before: createAuthMiddleware(async (request) => {
        // This package only issues identity tokens for its configured applications. Reject resource
        // indicators while OAuth Provider 1.6 has GHSA-p2fr-6hmx-4528.
        if (
          request.path?.startsWith("/oauth2/") &&
          (request.body?.resource !== undefined ||
            new URL(
        ...
        if (request.path === "/sign-up/email") {
          const allowed: boolean = await ctx.runQuery(
            internal.management.canRegister,
            {
              email: String(request.body?.email ?? ""),
              token: request.headers?.get("x-ovela-invite") ?? "",
            },
          );
          if (!allowed)
            throw new APIError("FORBIDDEN", {
    ...
    plugins: [
      ...
      jwt({
        ...
        adapter: {
          getJwks: async () => {
            const keys: Doc<"oidcKeys">[] = await ctx.runQuery(
              internal.sso.oidcKeys,
              {},
            );
          ...
          createJwk: async (key) => {
            if (!("runMutation" in ctx))
              throw new Error("Key creation requires a mutation context.");
            const record: Doc<"oidcKeys"> = await ctx.runMutation(
              internal.sso.createOidcKey,
              {
      ...
      oauthProvider({
        ...
        validAudiences: [`${process.env.SITE_URL}/api/auth`],
        ...
        postLogin: {
          ...
          consentReferenceId: async ({ user }) => {
            await assertClientAccess(ctx, user.id, authorizationClientId);
            return undefined;
          },
        ...
        customIdTokenClaims: async ({ user, metadata }) =>
          applicationClaims(ctx, user.id, metadata?.ovelaProvider),
        customAccessTokenClaims: async ({ user, metadata }) => {
          if (!user) throw new APIError("FORBIDDEN");
          await assertClientAccess(ctx, user.id, metadata?.ovelaProvider);
          return {};
        },
        customUserInfoClaims: async ({ user, jwt }) =>
          applicationClaims(ctx, user.id, jwt.client_id ?? jwt.azp),
      }),
    ],
...
export const createAuth = (ctx: GenericCtx<DataModel>) => {
  ...
  return {
    ...
    handler: async (request: Request) => {
      // Each HTTP request gets its own authorization context, including the provider's
      // signed login-resume query. Token claims independently use persisted client metadata.
      const url = new URL(request.url);
      let clientId = url.pathname.endsWith("/oauth2/authorize")
      ...
      if (
        ...
        if (type.includes("application/json")) {
          ...
          if (typeof body?.oauth_query === "string")
            clientId =
              new URLSearchParams(body.oauth_query.replace(/^\?/, "")).get(
                "client_id",
              ) ?? undefined;
        }
      }
      return betterAuth(createAuthOptions(ctx, clientId)).handler(request);
    },
  };
};
async function assertClientAccess(
  ...
  if (typeof clientId !== "string" || !["immich", "pelican"].includes(clientId))
    throw new APIError("FORBIDDEN", {
      message: "This application is not configured for Ovela sign-in.",
    });
  const active: boolean = await ctx.runQuery(internal.sso.activeClient, {
    clientId,
  });
  if (!active)
    throw new APIError("FORBIDDEN", {
  ...
  const allowed: boolean = await ctx.runQuery(internal.sso.canAccessClient, {
    authId,
    clientId,
  });
  if (!allowed)
    throw new APIError("FORBIDDEN", {
      message: `You do not have access to ${clientId === "immich" ? "Photos" : "Games"}.`,
    });
}
async function applicationClaims(
  ctx: GenericCtx<DataModel>,
  authId: string,
  clientId: unknown,
) {
  await assertClientAccess(ctx, authId, clientId);
  const profile: { name: string; picture: string | null } | null =
    await ctx.runQuery(internal.sso.photosProfile, { authId });
  const identity = profile
    ...
    : {};
  // Pelican owns game-server and administrator roles; an Ovela admin is not a Pelican admin.
  if (clientId !== "immich") return identity;
  const role: string = await ctx.runQuery(internal.sso.photosRole, { authId });
  return { immich_role: role, ...identity };
}
