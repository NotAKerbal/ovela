<script lang="ts">
  ...
  import { ActionButton, CommandPaletteDefaultProvider, ImageCarousel, Icon } from '@immich/ui';
  import { mdiImageOutline, mdiDotsVertical } from '@mdi/js';
...
<UserPageLayout hideNavbar={assetMultiSelectManager.selectionActive} scrollbar={false}>
  ...
  >
    ...
    {/if}
    {#snippet empty()}
      <div class="ovela-empty"><Icon icon={mdiImageOutline} size="64" /><h2>Your photo library starts here.</h2><p>Bring your favorite moments together. Upload your first photos to get started.</p><button class="ovela-upload" onclick={() => openFileUploadDialog()}>{$t('upload')}</button></div>
    {/snippet}
  </Timeline>
</UserPageLayout>
