...
<script lang="ts">
  ...
  import { ovelaProfile } from '$lib/managers/ovela-profile.svelte';
  ...
  const profile = $derived(user.id === authManager.user?.id ? ovelaProfile.current : null);
  const imageUrl = $derived(profile ? profile.picture : user.profileImagePath ? getProfileImageUrl(user) : null);
...
<figure
  class="{sizeClass} {colorClass} {interactiveClass} overflow-hidden rounded-md shadow-sm"
