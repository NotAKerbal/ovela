<script lang="ts">
  ...
  afterNavigate(() => sidebarStore.reset());
...
<nav
  ...
  tabindex="-1"
  class="ovela-sidebar relative z-1 w-0 immich-scrollbar overflow-x-hidden overflow-y-auto bg-light pt-8 transition-all duration-200 sidebar:w-64"
...
>
  <div class="ovela-sidebar-inner flex h-max min-h-full flex-col gap-1">
    {@render children?.()}
  </div>
</nav>
