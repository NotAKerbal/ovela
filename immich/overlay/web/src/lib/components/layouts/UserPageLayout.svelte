...
<script lang="ts">
  ...
  import OvelaMobileNav from '$lib/components/shared-components/navigation-bar/OvelaMobileNav.svelte';
...
<div class="ovela-user-shell" class:ovela-photos-page={page.url.pathname.startsWith('/photos')} class:ovela-selecting={hideNavbar}>
<header>
  {#if !hideNavbar}
...
<div
  tabindex="-1"
  class="ovela-user-layout relative z-0 grid grid-cols-[--spacing(0)_auto] overflow-hidden sidebar:grid-cols-[--spacing(64)_auto]
...
>
  ...
  <main class="ovela-content relative">
    <div class="ovela-page-scroll {scrollbarClass} absolute {hasTitleClass} w-full overflow-y-auto p-2" use:useActions={use}>
      {@render children?.()}
    </div>
...
{#if !hideNavbar}<OvelaMobileNav />{/if}
</div>
