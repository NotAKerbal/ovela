import { ovelaMobileGrid } from './ovela-grid';
...
export function getJustifiedLayoutFromAssets(
  ...
  // A stable three-column crop on phones; desktop retains Immich's justified rows.
  if (options.rowWidth > 0 && options.rowWidth < 768) {
    return ovelaMobileGrid(assets.length, options.rowWidth);
  }
  if (useWasm) {
