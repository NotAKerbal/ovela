...
Before the first start, copy `.env.example` to `.env.selfhost` and change the three public origins. For example:
...
Point your reverse proxy at host ports 3000, 3210, and 3211 respectively. Enable WebSocket forwarding for the Convex endpoint. Use HTTPS for all three public origins. The browser must be able to reach the public Convex URL; container-only names such as `backend` will not work there. No managed reverse proxy is required.

The default host bindings only accept local connections. If your reverse proxy runs on a different machine or needs published Docker host ports, set `BIND_ADDRESS` to your host's LAN address. Keep the admin key private; the Convex endpoint serves both authenticated application traffic and administrative requests protected by that key.

Port bindings are configurable using `APP_PORT`, `CONVEX_PORT`, and `CONVEX_SITE_PORT`. If you use direct LAN access, include the matching ports in all three public URLs. Rerun `./ovela up` after editing origins because Next.js compiles public variables into its browser bundle. Keep origins free of trailing slashes.
...
This first release manages Ovela accounts and application visibility. It does not yet provide a universal SSO provider or enforce permissions inside unrelated applications. Those applications still need their own access controls.
